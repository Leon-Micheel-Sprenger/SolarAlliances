...
INSERT INTO `solo_missions` (`Solo_Missions_Id`, `Name`, `Story`, `Time`, `Input_Money`, `Input_People`, `Input_Ore`, `Input_Water`, `Ships`, `Reward_Money`, `Reward_People`, `Reward_Ore`, `Reward_Water`, `Rank`) VALUES
(1, 'Cargo Transport', 'Description and story', '00:00:30', NULL, 10, NULL, 20, 5, 300, NULL, NULL, NULL, 1);
...
CREATE TABLE `submited_mmissions` (
  ...
  `Submited_ship` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
ALTER TABLE `ship_fleet`
  ...
  ADD KEY `Spaceships` (`SpaceshipsId`);
...
ALTER TABLE `submited_mmissions`
  ADD PRIMARY KEY (`Id`),
  ADD KEY `Submited_ship` (`Submited_ship`);
...
--
ALTER TABLE `solo_missions`
  MODIFY `Solo_Missions_Id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
...
--
ALTER TABLE `ship_fleet`
  ADD CONSTRAINT `ship_fleet_ibfk_1` FOREIGN KEY (`PlayerId`) REFERENCES `player` (`Player_Id`),
  ADD CONSTRAINT `ship_fleet_ibfk_2` FOREIGN KEY (`SpaceshipsId`) REFERENCES `spaceships` (`Spaceships_Id`);

--
-- Constraints for table `solo_missions`
--
ALTER TABLE `solo_missions`
  ADD CONSTRAINT `solo_missions_ibfk_1` FOREIGN KEY (`Ships`) REFERENCES `spaceships` (`Spaceships_Id`);
...
--
ALTER TABLE `submited_mmissions`
  ADD CONSTRAINT `submited_mmissions_ibfk_1` FOREIGN KEY (`Submited_ship`) REFERENCES `ship_fleet` (`Ship_Fleet_ID`);
COMMIT;
